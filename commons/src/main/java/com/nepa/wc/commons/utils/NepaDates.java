...
public final class NepaDates {
    private static final Logger LOG = LoggerFactory.getLogger(NepaDates.class);
    ...
    public static Date min(Date... dates) {
        dates = NepaArrays.notEmpty(dates);
    ...
    public static Date date(Integer... fields) {
        ...
        for (int i = 0; i < FIELDS.length; i++) {
            val = NepaArrays.get(fields, i, (Integer) 0);
    ...
    public static Date firstDayOfWeek(Date date,Integer hourSpan){
        ...
        if(date == null) date = NepaDates.utcDate(hourSpan);
        else date = NepaDates.add(date,Calendar.HOUR_OF_DAY,hourSpan);
        return NepaDates.setFields(date,Calendar.DAY_OF_WEEK,calendar().getFirstDayOfWeek());
    }
...
//                cal.set(Calendar.DAY_OF_WEEK,Calendar.SUNDAY);
    ...
    public static Date utcWithHourOffset(Date date, int hourOffset) {
        date = date == null ? NepaDates.utc() : NepaDates.utc(date);
