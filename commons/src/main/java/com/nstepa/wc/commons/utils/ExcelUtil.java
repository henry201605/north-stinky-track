...
import com.nstepa.wc.commons.annotations.ExcelResources;
...
public class ExcelUtil {
	 ...
	 * @param isClasspath 模板是否在classPath路径下
	 */
	public void exportObj2ExcelByTemplate(Map<String, String> datas, String template, String outPath, List objs, Class
            clz, boolean isClasspath) {
		ExcelTemplate et = handlerObj2Excel(template, objs, clz, isClasspath);
		et.replaceFinalData(datas);
	 ...
	 * @param clz     对象类型
	 */
	public String exportObj2Excel2(String outPath, List objs, Class clz) throws Exception {
		isExistDir(outPath);
	 ...
	 * @param clz     对象类型
	 */
	public void exportObj2Excel(String outPath,List objs,Class clz) throws Exception{
		isExistDir(outPath);
		Workbook wb = handleObj2Excel(objs, clz);
		FileOutputStream fos = null;
		try {
	 ...
	 */
	public static void isExistDir(String filePath) throws Exception {
		String paths[] = {""};
		String tempPath = "";
		//切割路径
		try {
			tempPath = new File(filePath).getCanonicalPath();//File对象转换为标准路径并进行切割，有两种windows和linux
			paths = tempPath.split("\\\\");//windows
			if (paths.length == 1) {
				paths = tempPath.split("/");
			}//linux
		} catch (IOException e) {
			throw new Exception("切割路径错误");
		}
		//判断是否有后缀
		boolean hasType = false;
		if (paths.length > 0) {
			String tempPath1 = paths[paths.length - 1];
			if (tempPath1.length() > 0) {
				if (tempPath1.indexOf(".") > 0) {
					hasType = true;
				}
			}
		}
